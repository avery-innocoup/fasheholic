...
$_REQUEST['product_cat'],
$_REQUEST['product_desc'],
$_REQUEST['product_myr_price'],
$_REQUEST['product_usd_price'],
$_REQUEST['product_image_base64'],
$_REQUEST['product_image_name'],
$_REQUEST['product_image_ext']
)) {
	$errorid = 3;
...
if($errorid == 0) {
	$product_name = clear_sql_inj($_REQUEST['product_name'], $link);
	$product_cat = clear_sql_inj($_REQUEST['product_cat'], $link);
	$product_desc = clear_sql_inj($_REQUEST['product_desc'], $link);
	$product_myr_price = clear_sql_inj($_REQUEST['product_myr_price'], $link);
	$product_usd_price = clear_sql_inj($_REQUEST['product_usd_price'], $link);
	$product_image_base64 = $_REQUEST['product_image_base64'];
	$product_image_name = clear_sql_inj($_REQUEST['product_image_name'], $link);
	$product_image_ext = clear_sql_inj($_REQUEST['product_image_ext'], $link);
