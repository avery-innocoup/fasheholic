...
if($errorid == 0) {
	$email = clear_sql_inj($_REQUEST['email'], $link);
	$name = clear_sql_inj($_REQUEST['name'], $link);
	$username = clear_sql_inj($_REQUEST['username'], $link);
	$password = clear_sql_inj($_REQUEST['password'], $link);
